...
2
O passo seguinte vai implicar mudar do modo authconfig para o authselect (a máquina em questão estava ainda com authconfig), por isso será necessário utilizar o --force. Executar o seguinte no diretório casa (home) da root.
	
	authselect --trace select --force sssd  with-mkhomedir > change-authselect-with-sssd.log  2>&1

3
Colocar em /etc/sssd/sssd.conf:
	...
	ldap_search_base = dc=nome_grupo,dc=ads,dc=dcc
